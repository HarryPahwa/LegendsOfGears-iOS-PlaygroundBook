...
public class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func loadSolution() {
        ...
        locations.append(CGPoint(x:165, y: 268))
        locations.append(CGPoint(x:350, y: 344.5))
        locations.append(CGPoint(x:521.5, y: 390))
        
        let gear1 = makeAGear(37, locations[0], UIColor.yellow)
        
        let gear2 = makeAGear(39, locations[1], UIColor.red)
        
        puzzleGear = makeAGear(30, locations[2], UIColor.purple)
        puzzleGear!.alpha = 0.2
        
        
        for joint in pinJointsBodies {
            
            if gear1 == joint.pin || gear2 == joint.pin {
                thingsShouldHover(joint.pin, PhysicsCategory.gearLevel)
                thingsShouldHover(joint.gear, PhysicsCategory.gearLevel)
            } else if puzzleGear == joint.pin {
                joint.gear.alpha = 0.2
            }
        }
    }
...
//        addChild(ferrisWheel)
    ...
    public func makeAGear(_ teeth: Int,_ positionInScene: CGPoint,_ gearColor: UIColor = .randomColor()) -> SKNode {
        ...
        if teeth == 0 {
            gearAddition = Gear(teeth: Int(randRange(lower: 20, upper: 35)), module: 5)
        } else {
